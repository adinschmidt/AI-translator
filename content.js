...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  switch (request.action) {
    ...
    case "replacePageContent":
       ...
       } else if (!isTranslated) { // Only translate if not already translated
...
function extractMainContentText() {
    ...
    clonedBody.querySelectorAll('script, style, nav, header, footer, aside, form, button, input, textarea, select, [aria-hidden="true"], noscript').forEach(el => el.remove());
...
function replaceVisibleText(fullTranslation) {
    ...
    const translatedSegments = fullTranslation.split('\n').filter(s => s.trim().length > 0);
    let segmentIndex = 0;
        ...
        { // Filter function
            acceptNode: function(node) {
                ...
                // Skip nodes that are part of the extension's UI (popup, indicator)
                 if (node.parentElement && (node.parentElement.closest('#translation-popup-extension') || node.parentElement.closest('#translation-loading-indicator'))) {
                     return NodeFilter.FILTER_REJECT;
                 }
    ...
    nodesToReplace.forEach(node => {
        if (segmentIndex < translatedSegments.length) {
            ...
            if (node.nodeValue.trim().length > 5) { // Arbitrary length check
                 console.log(`Replacing node value: "${node.nodeValue.trim().substring(0,50)}..." with segment ${segmentIndex}`);
    ...
    if (segmentIndex < translatedSegments.length) {
        console.warn(`Not all translated segments were used (${segmentIndex}/${translatedSegments.length}). Page structure might not match.`);
    }
            ...
            // Need API key/endpoint info to proceed, request from background
             chrome.runtime.sendMessage({ action: "triggerAutoTranslate" }, (response) => {
                 if (chrome.runtime.lastError) {
                     console.error("Error sending triggerAutoTranslate:", chrome.runtime.lastError.message);
                     ...
                     console.warn("Background script did not confirm auto-translate start. Is it configured?", response);
                 }
             });
