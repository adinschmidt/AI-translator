...
- **API Keys:** Your API keys are stored exclusively on your device using Chrome's synchronized storage (`chrome.storage.sync`). This means your settings sync across your Chrome browsers when signed in, but they are never transmitted to the developer or any third party other than the specific AI provider you choose for translation requests.
...
- API keys are stored in Chrome's secure storage and are only transmitted over HTTPS to the AI provider endpoints.
